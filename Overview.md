This project is a static landing page for MSV Company Profile.
Requirement by client
- Custom responsive UI (desktop & mobile)
- Brand-aligned colors & typography
- Core pages setup
- Basic on-page SEO
- Admin handover
→ DELIVERABLES
- Live website
- Full admin access (need clarification, for now it's just like deliver setup guide, the site is still static)

Developement preference from me
- NextJS
- TailwindCSS for styling
- Vercel for hosting
- Read context docs to extract content at /Context folder
- Read THEME.md for brand colors & typography 
- Take image and logos from /References folder